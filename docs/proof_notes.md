...
> **【2026-09-22 追記】** 本表の「$\Omega(1/\alpha)$（$\alpha N\ge C$）」「$\beta=1$ で比 $\Theta(1/\alpha)$」の2行は、1巡目査読対応（v2）で**撤回済み**（詳細は各行を参照）。現行の定理化は `theorem_41_v4.tex`（3巡目査読 fable_points3.md 反映版）を正本とし、主定理は「固定 $\alpha$・$N\to\infty$・$\beta\in(1,2)$」に限定、共同極限は Zador 漸近の $\alpha$ 一様性（仮定 (H)）を明示した系に降格している。以下の表は v1 時点の作業記録として残す。

% ======================================================================
% 予想4.1 の定理化 — 卒論用の清書 v5（4巡目査読 P9v4 反映版）
% v4 からの変更点（4巡目査読 fable_points4.txt に対応．いずれも表現・厳密性の微修正で，主張・構造は不変）:
% (a) 補題3.1 部分区間: 中間不等式 |F^{-1}(τ)-q| ≤ F^{-1}(Mh+h/2)-F^{-1}(Mh) は非減少性だけからは従わない
%     （Mh+h/2<α のとき τ>Mh+h/2 で右側超過が起こる）ため，τ と Mh+h/2 が共に [α-h, α+h/2] に属することから直接評価する形に修正
% (b) 補題3.1: h_0 の定め方に「O(h) 項が h^{2-β}ℓ(h) の定数倍で吸収される範囲」を追加（上界定数 c_2 の α 非依存性の根拠を明示）
% (c) 定理1.1: 定数の α 非依存性の根拠を「補題3.1 の一様評価」から「補題3.1 の証明（固定 α では o 項の吸収）」に修正
%     （一様評価 (14) の条件 α+h≤α_0 は α=α_0 の固定 α では満たされないため）
% (d) 注意1.3（β=1）: Darboux 上下界がともに (h/4)ℓ_1(h) に漸近することから ≍ を ~ に強化（比の定数 4 は消え，1 になる）
% (e) 注意3.1: 一様収束の範囲を「(0,∞) のコンパクト部分集合」と明記
% (f) 補題3.3(ii): β=1 で ℓ_1 が有界な場合（F^{-1} 有界）を明記
% (g) 例1.2: α=1 は定義1.1 の α∈(0,1) の範囲外だが，用いるのは補題3.3 の Zador 条件のみであることを注記
% (h) 注意1.2: (H') の証明方針に，特異点を含む第1区間と inf/sup 比の制御が別途必要である旨の留保を追加
% (i) ヘッダコメントの注意番号（(7)(9)）を実番号に修正
% ----------------------------------------------------------------------
...
% (7) 注意1.3（β=1）: 第1区間は hℓ(h) のオーダーにとどまり，ℓ_1 因子は正則部から生じることを明記
% (8) 注意3.1（新設）: 主定理の ≍ を ~（明示定数 K_β）へ強化する方針の概略
% (9) 注意1.2（新設）: 仮定 (H) の弱形 (H') の位置づけ
...
評価が有効になる閾値 $N\ge N_0$ は $\alpha$ に依存してよいが，定数は依存しない
（補題\ref{lem:unif} の証明において，第1区間と正則部の上下界の定数は $\Finv$ のみから定まり，$\alpha$ に依存する項はすべて $o(h^{2-\beta}\ell(h))$ で $N\ge N_0(\alpha)$ 以降に吸収される）．
...
（$X_1'$ の分位点関数 $-cu^{-(\beta-1)}$ は $(0,1]$ 上で $C^1$・凹・$\ell\equiv c(\beta-1)$ の正則変動をもち，
補題\ref{lem:zador-cond} の証明は $\alpha=1$ でもそのまま通用するので，補題\ref{lem:zador} が適用できる．
定義\ref{def:regvar} は $\alpha\in(0,1)$ で述べたが，ここで必要なのは Zador の条件（絶対連続性・モーメント・$\int f^{1/2}<\infty$）のみである）．
...
ただし，特異点 $\tau=0$ を含む第1区間は補題\ref{lem:midpoint} の適用範囲外であり補題\ref{lem:unif} の第1区間と同種の直接評価を要すること，
および各区間上での $\sup(\Finv)'/\inf(\Finv)'$ の比を（原点近傍では正則変動により，それ以外では有界性により）制御する必要があることに注意する．
...
\begin{equation*}
  \Eunif\;\sim\;\frac1{4\alpha}\,\frac{\ell_1(1/N)}{N},\qquad
  \Eopt\;\sim\;\frac1{4\alpha N}\left(\int_0^\alpha\sqrt{(\Finv)'(\tau)}\,d\tau\right)^{2},\qquad
  \frac{\Eunif}{\Eopt}\;\sim\;\frac{\ell_1(1/N)}{\bigl(\int_0^\alpha\sqrt{(\Finv)'}\bigr)^{2}}
\end{equation*}
...
$\int_0^{h/2}\bigl(\Finv(h/2)-\Finv(\tau)\bigr)\,d\tau=\int_0^{h/2}s\,(\Finv)'(s)\,ds\sim\int_0^{h/2}\ell(s)\,ds\sim\tfrac h2\ell(h)$
（Fubini と補題\ref{lem:karamata}\eqref{eq:karamata1}，$p=0$）および同様の右側の項により $I_0\asymp h\,\ell(h)=o\bigl(h\,\ell_1(h)\bigr)$ にとどまり，
主要項は正則部の Darboux 型評価
$\frac h4\bigl(\Finv(\alpha)-\Finv(2h)\bigr)\le\sum_{i=1}^{M-1}e_i\le\frac{h^2}4(\Finv)'(h)+\frac h4\bigl(\Finv(\alpha)-\Finv(h)\bigr)$
から生じる．ここで $-\Finv(2h)\sim-\Finv(h)\sim\ell_1(h)$（$\ell_1$ は緩変動），$h^2(\Finv)'(h)\sim h\ell(h)=o(h\ell_1(h))$，$h\Finv(\alpha)=O(h)=o(h\ell_1(h))$ であるから，
上下界はともに $\tfrac h4\ell_1(h)$ に漸近し，$\sum_{i=1}^{M-1}e_i\sim\tfrac h4\ell_1(h)$ が従う．部分区間の寄与は補題\ref{lem:unif} と同様に $o(h)$ であるから，
$\beta>1$ の場合の $\asymp$ とは異なり $\beta=1$ では $\Iunif\sim\tfrac h4\ell_1(h)$ という $\sim$ 評価（定数 $\tfrac14$）が直接得られる．
...
本稿の主定理の仮定は $C^1$    みであるため，これは参考情報にとどめ，証明には補題\ref{lem:midpoint} の $C^1$ 版（式\eqref{eq:midpoint-abs}）のみを用いる．
\end{remark}
...
以下，$h\le h_0$ の $h_0$ は，式\eqref{eq:regvar}・補題\ref{lem:tail} の漸近（および以下で用いる補題\ref{lem:karamata} の漸近）が相対誤差 $\varepsilon$ 以内で成り立ち，
かつ $h^{\beta-1}/\ell(h)\le1$（$\beta>1$ より $h\downarrow0$ で $\to0$；この条件により $O(h)$ の項が $h^{2-\beta}\ell(h)$ の定数倍で押さえられる）を満たす範囲として，$\Finv$ のみから定める．
...
$\alpha-h\le Mh\le\alpha$ より，$\tau\in[Mh,\alpha]$ と $Mh+\frac h2$ はともに区間 $[\alpha-h,\alpha+\frac h2]$（$\alpha+\frac h2<1$）に属する．
よって $\Finv$ の非減少性から
\begin{equation*}
 \Finv(\alpha-h)\;\le\;\Finv(\tau),\;q\;\le\;\Finv(\alpha+\tfrac h2),
 \qquad\text{したがって}\qquad
 |\Finv(\tau)-q|\;\le\;\Finv(\alpha+\tfrac h2)-\Finv(\alpha-h)
\end{equation*}
であり，寄与は $\le w\bigl(\Finv(\alpha+\frac h2)-\Finv(\alpha-h)\bigr)\le h\bigl(\Finv(\alpha+\frac h2)-\Finv(\alpha-h)\bigr)$ である
（$Mh+\frac h2<\alpha$ のときは $\tau>Mh+\frac h2$ で $\Finv(\tau)>q$ となり得るため，$q-\Finv(Mh)$ だけで押さえるのではなく両側から挟む）．
\begin{itemize}
...
\item 一様評価\eqref{eq:unif-uniform}の場合：$\alpha+h\le\alpha_0$ より $[Mh,Mh+h]\subset(0,\alpha_0]$ で $\varphi$ は非増加・有限であるから，
 平均値の定理により $|\Finv(\tau)-q|\le|\tau-(Mh+\tfrac h2)|\,\varphi(Mh)\le h\,\varphi(Mh)\le h\,\varphi(\alpha/2)$
 （$\tau,\,Mh+\frac h2\in[Mh,Mh+h]$ と，$2h\le\alpha$ から $Mh\ge\alpha-h\ge\alpha/2$），寄与は $\le w\,h\,\varphi(\alpha/2)\le h^2(\Finv)'(\alpha/2)$．
\end{itemize}
...
補題\ref{lem:tail} より $\Finv(hu)/(h^{1-\beta}\ell(h))\to-\frac{u^{-(\beta-1)}}{\beta-1}$ が $(0,\infty)$ の任意のコンパクト部分集合上で $u$ について一様に成り立つ
（$u\downarrow0$ の近傍は補題\ref{lem:unif} の第1区間と同様に補題\ref{lem:karamata} で別途処理する）．
...
$\beta<2$ よりこのような $\delta>0$ が存在する．$\beta=1$ では，$\ell_1\to\infty$ なら $|\Finv|\sim\ell_1$ が緩変動なので任意の $\delta$ で可積分であり，
$\ell_1$ が有界なら $\Finv$ 自身が $(0,\alpha]$ で有界（$x_0>-\infty$）なので自明に可積分．
